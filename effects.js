...
export function updateEffectsImage() {
  if (!state.originalImageData || !effectsCanvas.width || !effectsCanvas.height) return;

  // Effects possui uma fonte independente: sempre parte da X-RAY original.
...
function calculateHistogram() {
  ...
  if (!state.originalImageData) return histogram;
  // O histograma de Effects também é calculado exclusivamente sobre
  // a imagem X-RAY original, mantendo as janelas totalmente isoladas.
...
export function drawEffectsControls() {
  if (effectsPanel.hidden || !state.originalImageData) return;
  drawRangeBar();
  drawHistogram();
}
